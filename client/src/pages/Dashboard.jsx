    ...
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link
            to="/home"
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg transition-all cursor-pointer"
          >
            + Create New
          </Link>
        </div>
        ...
        <div className="bg-opacity-5 backdrop-blur-lg rounded-xl overflow-hidden border border-white border-opacity-10">
          {/* Table Header */}
          <div className="grid grid-cols-12 gap-4 px-6 py-4 bg-opacity-5 border-b border-gray-200 text-gray-300 font-medium text-sm">
          ...
          <div>
            {urls.map((url) => (
              ...
              <div
                key={url._id}
                className="grid grid-cols-12 gap-4 px-6 py-4 border-b-1 border-gray-700 hover:bg-gray-600/30 transition-colors"
              >
                ...
                <div className="col-span-4 text-gray-400 truncate flex gap-2 items-center justify-start gap-3 cursor-pointer">
                  <CopyIcon
                    ...
                    className="h-4 w-4 hover:text-indigo-400 "
                  />
                  <a
                    href={url.longUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hover:text-indigo-400 tracking-tighter text-sm hover:underline transition-colors"
                  >
                    {url.longUrl}
                  </a>
                </div>
                <div className="col-span-2 text-gray-400 truncate flex gap-2 items-center justify-start gap-2 cursor-pointer">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hover:underline tracking-tighter hover:text-indigo-400 text-smtransition-colors"
                  >
                    {import.meta.env.VITE_BACKEND_URL + "/" + url.shortUrl}
                  </a>
                </div>
                <div className="col-span-2 text-gray-400 truncate flex items-center justify-start gap-2 cursor-pointer">
                  <QrCodeIcon className="h-4 w-4 text-indigo-400" />
                  <button
                    onClick={() => {
                      const a = document.createElement("a");
                      a.href = url.qrCode; // Use the QR code for this URL
                      a.download = `${url.customShort || url.shortUrl}-qr.png`;
                      a.click();
                    }}
                    className="text-gray-400 hover:text-indigo-400 transition-colors p-1 cursor-pointer"
                  >
                    Download
                  </button>
                </div>
